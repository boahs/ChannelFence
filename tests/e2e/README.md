...
- `regression/` covers known bugs and secondary surfaces, including late-populated and hidden search bylines, description mentions, collaboration owners, linkless right-rail recommendations, compact Shorts cards and menus, feed-wide Shorts hiding, and manual handle entry.
- `live/` verifies injection plus current search-result, compact Shorts blocking, and watch-page right-rail compatibility against the real YouTube origin; it is not part of the normal local or pull-request run.
