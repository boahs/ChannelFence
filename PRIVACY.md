...
While you use YouTube, ChannelFence reads limited page content needed to identify creator names and channel links, determine whether a creator is on your block list, add block controls, and hide matching content. When you choose to block a creator, ChannelFence stores that creator’s displayed name, public channel identifier or handle, public channel URL, and the time the block was created.

## Storage and transmission

Settings and blocked-creator records are stored only in Chrome’s local extension storage on your device. ChannelFence does not transmit this information to the developer or to any third party. It does not sell data, use data for advertising, or permit humans to read user data.

## Permissions

- **Storage:** saves the enabled state, comment preference, and blocked-creator list locally.
